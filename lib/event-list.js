...
EventList.prototype = {
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   /**
    * You should considere using childworkflow_terminated_or_failed instead to cover both cases
   ...
   get_last_marker_details: function (markerName) {
    ...
    /**
     * Return true if the global workflow was timed out ==> NOT WORKING since aws do not notify the decider
     * @returns {boolean}
     */
    workflow_execution_timedout : function(){
        var i;
        for (i = 0; i < this._events.length; i++) {
            var evt = this._events[i];
            if (evt.eventType === "WorkflowExecutionTimedOut") {
                return true;
            }
        }
        return false;
     ...
     * @returns {Boolean}
     */
    childworkflow_terminated: function(control) {
        var initiatedEventId, initiatedEvent;
        return this._events.some(function (evt) {
            if (evt.eventType === "ChildWorkflowExecutionTerminated") {
                initiatedEventId = evt.childWorkflowExecutionTerminatedEventAttributes.initiatedEventId;
                initiatedEvent = this.eventById(initiatedEventId);
                if (initiatedEvent.startChildWorkflowExecutionInitiatedEventAttributes.control === control) {
                    return true;
                }
    ...
    childworkflow_terminated_or_failed : function(control){
            var self = this;
            return self.childworkflow_failed(control) || self.childworkflow_terminated(control);
    }
